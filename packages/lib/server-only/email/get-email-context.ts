...
export const getEmailContext = async (options: GetEmailContextOptions): Promise<EmailContextResponse> => {
  ...
  // system mailer + BringSpark sender, so the degraded organisation is findable.
  ...
  const resolvedTransportData = transportResolution
    ...
    : {
        name: BRINGSPARK_INTERNAL_EMAIL.name,
        address: BRINGSPARK_INTERNAL_EMAIL.address,
        transport: mailer,
      };
  ...
  const senderEmailId = match(meta?.emailId)
    ...
    .with(null, () => null) // Explicit null means to use the BringSpark email.
    .exhaustive();
