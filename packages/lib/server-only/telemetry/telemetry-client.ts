...
const TELEMETRY_DISABLED = !!process.env.BRINGSPARK_DISABLE_TELEMETRY || HAS_LICENSE_KEY;

const NODE_ID_FILENAME = '.bringspark-node-id';
...
export class TelemetryClient {
   ...
   *
   * If telemetry is disabled via `BRINGSPARK_DISABLE_TELEMETRY=true` or credentials are not
  ...
  public static async start(): Promise<void> {
    if (TELEMETRY_DISABLED) {
      if (!HAS_LICENSE_KEY) {
        console.log(
          '[Telemetry] Telemetry is disabled. To enable, remove the BRINGSPARK_DISABLE_TELEMETRY environment variable.',
        );
      }
  ...
  private async initialize(): Promise<void> {
    ...
    console.log(
      '[Telemetry] Telemetry is enabled. BringSpark collects anonymous usage data to help improve the product.',
    );
    console.log(
    ...
    console.log(
      '[Telemetry] To disable telemetry, set BRINGSPARK_DISABLE_TELEMETRY=true in your environment variables.',
    );
    console.log('[Telemetry] Learn more: https://bringspark.com/docs/developers/self-hosting/telemetry');
