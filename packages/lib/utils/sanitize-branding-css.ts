 ...
 * CSS nesting (`.bringspark-branded { <user css> }`). The sanitiser itself
 ...
 */
export const SANITIZE_BRANDING_SCOPE_CLASS = 'bringspark-branded';
 ...
 * The sanitised output is later wrapped in `.bringspark-branded { ... }` via
...
const sanitizeRule = (rule: Rule, warnings: SanitizeBrandingCssWarning[]): void => {
  ...
  // the entire sanitised CSS in `.bringspark-branded { ... }` (CSS nesting).
