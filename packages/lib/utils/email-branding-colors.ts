 ...
 * token (tenant value or BringSpark default), so consumers never see `undefined`.
 *
 * Produced by `resolveEmailBrandingColors`, or `null` when the tenant has no
 * usable/safe colour set (callers fall back to the default BringSpark palette).
 */
 ...
 * otherwise the BringSpark default. We do NOT enforce contrast or readability —
 ...
 *
 * Returns `null` (⇒ caller uses the default BringSpark palette) only when there
